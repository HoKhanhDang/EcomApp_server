...
const getProducts = async (req, res) => {
    ...
    if (queryObj?.category) {
        formattedQuery.category = {
            $regex: `.*${queryObj.category}`,
            $options: "i",
        };
    }
    if (queryObj?.color) {
        const colors = Array.isArray(queryObj.color)
            ? queryObj.color
            : queryObj.color.split(",");
...
const getProductBySlug = async (req, res) => {
    ...
    if (!slug)
        return res.status(400).json({ message: "Please provide product slug" });
...
const ratingProduct = async (req, res) => {
    ...
    if (!star || !comment || !pid)
        return res
            .status(400)
            .json({ message: "Please provide star, comment and product ID" });
    ...
    const updateRating = await updatedProduct.updateOne({
